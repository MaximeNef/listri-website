...
// Only if not receiving a full recording, otherwise we might receive very old data and mess up session length
if (!isset($_GET['w'])) {
    $query = "UPDATE `ust_clientpage`, `ust_clients` 
            SET 
                ust_clientpage.last_activity = NOW(),
                ust_clients.last_date = NOW()
            WHERE 
            ust_clientpage.clientid = ust_clients.id AND
            ust_clientpage.id = :clientPageID";
